...
class Package {
	...
	#if macro
	public static function fuckexport( path : String = "package.json" ) : Void{
	...
	inline public static function addDependency( name : String , version : String ) : Void {
	...
	static inline var USAGE = "Usage: 'implements server.externs.npm.Require<\"module-name\",\"module-version\">'";
	static inline var NPM_REQUIRE = "server.externs.npm.Package.Require";
	static inline var NPM_REQUIRE_NAMESPACE = "server.externs.npm.Package.RequireNamespace";
	...
	static inline var NO_JS_REQUIRE = "noJsRequire";
		...
		var requireParams = server.externs.util.Macro.extractStringParams( cl , NPM_REQUIRE );
		if( requireParams.length == 0 ){
			requireParams = server.externs.util.Macro.extractStringParams( cl , NPM_REQUIRE_NAMESPACE );
			requireNS = true;
		}
		...
		if( requireParams.length > 0 ){
			...
			if( requireNS ){
				...
				var _nativeName = server.externs.util.Macro.extractNative( cl );
				if( _nativeName != null ){
			...
			if( !Context.defined(NO_JS_REQUIRE) ) {
				...
				if( isNpm ) 
					server.externs.npm.Package.addDependency('${required.name}','${required.version}');
...
@:autoBuild(server.externs.npm.Package.Include.build())
#if (haxe_ver >= 3.3)
